...
onready var _break_keys = [KEY_ENTER, KEY_E]
...
func add_break_key_by_scancode(i): # Set a new key to resume breaks (uses scancode!)
	_break_keys.append(i)

func remove_break_key_by_scancode(i): # remove a key to resume breaks (uses scancode!)
	_break_keys.remove(i)
